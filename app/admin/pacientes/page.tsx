...
type Paciente = {
  ...
  lastLoginAt: string | null; totalSesiones: number; totalCitas: number
  totalPagado: number; tieneDeuda: boolean; esNuevo: boolean
  sinSesionDias: number | null
...
export default function PacientesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/admin/pacientes?search=${encodeURIComponent(search)}&page=${page}&filtro=${filtro}`)
    ...
    <div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-6">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl">
            <div className="text-center mb-6">
              ...
              <p className="text-[#8a9b8e] text-sm mt-1">{resetResult.patientName}</p>
            </div>
            <div className="bg-[#f5f3ef] border border-[#e8dfd0] rounded-xl p-4 mb-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={copyPassword} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${copied ? 'bg-green-500 text-white' : 'bg-[#4a7c59] text-white hover:bg-[#3d6849]'}`}>
                  {copied ? '✓ Copiado' : 'Copiar'}
                </button>
              </div>
            </div>
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mb-5">
              <p className="text-yellow-800 text-xs font-semibold">⚠️ Esta contraseña no se mostrará nuevamente</p>
            </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
            ...
            className="pl-10 pr-4 py-2.5 border border-[#e8dfd0] rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#4a7c59] w-72 bg-white" />
        </div>
      </div>
      ...
      {kpis && (
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-6">
          {[
            { label:'Total', value: kpis.total, icon:'👥', color:'text-[#1a2e1e]', filtro:'todos' },
            { label:'Activos', value: kpis.activos, icon:'🟢', color:'text-green-600', filtro:'activos' },
            { label:'Nuevos (30d)', value: kpis.nuevos, icon:'🔵', color:'text-blue-600', filtro:'nuevos' },
            { label:'Con deuda', value: kpis.conDeuda, icon:'🟡', color:'text-yellow-600', filtro:'deuda' },
            { label:'Ingresos', value: `USD ${kpis.ingresos.toFixed(0)}`, icon:'💳', color:'text-[#4a7c59]', filtro:'' },
            { label:'Sesiones hoy', value: kpis.sesionesHoy, icon:'📅', color:'text-purple-600', filtro:'' },
          ].map(k => (
            <button key={k.label}
              onClick={() => k.filtro && setFiltro(k.filtro)}
              className={`bg-white rounded-xl border p-4 text-center transition-all ${k.filtro && filtro===k.filtro ? 'border-[#4a7c59] shadow-sm' : 'border-[#e8dfd0] hover:border-[#4a7c59]/40'} ${k.filtro ? 'cursor-pointer' : 'cursor-default'}`}>
      ...
      <div className="flex gap-2 mb-5 overflow-x-auto">
        {FILTROS.map(f => (
          <button key={f.id} onClick={() => { setFiltro(f.id); setPage(1) }}
            className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${filtro===f.id ? 'bg-[#1a2e1e] text-white' : 'bg-white border border-[#e8dfd0] text-[#5a6b5e] hover:border-[#1a2e1e]/40'}`}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {pacientes.map(p => {
            ...
            return (
              <div key={p.id} className={`bg-white rounded-2xl border transition-all ${isExpanded ? 'border-[#4a7c59]/40 shadow-md' : 'border-[#e8dfd0] hover:border-[#4a7c59]/20'}`}>
                {/* Fila principal */}
                <div className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    {p.esNuevo && <div className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-blue-500 rounded-full border-2 border-white" />}
                  </div>

                  {/* Info */}
                  <div className="flex-1 min-w-0 grid grid-cols-6 gap-3 items-center cursor-pointer" onClick={() => setExpanded(isExpanded ? null : p.id)}>
                    ...
                    <div>
                      <span className={`text-xs px-2.5 py-1 rounded-full font-semibold border ${badge.cls}`}>{badge.label}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    {/* Ver ficha */}
                    <Link href={`/admin/pacientes/${p.id}`} title="Ver ficha completa"
                      className="w-8 h-8 rounded-lg bg-[#f0f7f2] hover:bg-[#4a7c59] hover:text-white flex items-center justify-center text-[#4a7c59] transition-colors text-sm">
                      📋
                    </Link>
                    {/* Reset password */}
                    <button onClick={() => resetPassword(p)} disabled={resetting === p.id} title="Restablecer contraseña"
                    ...
                    {/* Toggle activo */}
                    <button onClick={() => toggleActivo(p.id, p.isActive)} disabled={toggling === p.id} title={p.isActive ? 'Desactivar' : 'Activar'}
                    ...
                    <button onClick={() => setExpanded(isExpanded ? null : p.id)}
                      className="w-8 h-8 rounded-lg bg-[#f5f3ef] hover:bg-[#e8dfd0] flex items-center justify-center text-[#8a9b8e] transition-colors text-xs">
                      {isExpanded ? '▲' : '▼'}
                    </button>
                  </div>
                </div>
                    ...
                    <div className="grid md:grid-cols-4 gap-5">
                      ...
                      <div>
                        ...
                        <div className="space-y-1.5 text-xs">
                          <div className="flex justify-between"><span className="text-[#8a9b8e]">Registrado</span><span className="text-[#1a2e1e] font-medium">{fmtDate(p.createdAt)}</span></div>
                          <div className="flex justify-between"><span className="text-[#8a9b8e]">Último acceso</span><span className="text-[#1a2e1e] font-medium">{fmtDateTime(p.lastLoginAt)}</span></div>
                          <div className="flex justify-between"><span className="text-[#8a9b8e]">Última sesión</span><span className="text-[#1a2e1e] font-medium">{p.ultimaCita ? fmtDate(p.ultimaCita.scheduledAt) : '—'}</span></div>
                          <div className="flex justify-between"><span className="text-[#8a9b8e]">Total citas</span><span className="text-[#1a2e1e] font-medium">{p.totalCitas}</span></div>
                          {p.sinSesionDias !== null && (
                            <div className="flex justify-between">
                              <span className="text-[#8a9b8e]">Sin sesión hace</span>
                              <span className={`font-semibold ${p.sinSesionDias > 30 ? 'text-orange-500' : 'text-[#4a7c59]'}`}>{p.sinSesionDias}d</span>
                            </div>
                          )}
                        </div>
                      </div>
                        ...
                        {p.proximaCita ? (
                          <div className="bg-white border border-green-200 rounded-xl p-3">
                            <p className="font-semibold text-[#1a2e1e] text-sm">
                              {new Date(p.proximaCita.scheduledAt).toLocaleDateString('es-AR',{weekday:'long',day:'numeric',month:'short'})}
                            </p>
                            <p className="text-[#8a9b8e] text-xs mt-0.5">
                              {new Date(p.proximaCita.scheduledAt).toLocaleTimeString('es-AR',{hour:'2-digit',minute:'2-digit'})} hs · {p.proximaCita.therapist}
                            </p>
                      ...
                      {/* Alertas */}
                      <div>
                        <p className="text-xs font-semibold text-[#8a9b8e] uppercase tracking-wide mb-3">Alertas</p>
                        <div className="space-y-2">
                          {p.sinSesionDias !== null && p.sinSesionDias > 30 && (
                            <div className="p-2.5 bg-orange-50 border border-orange-200 rounded-xl">
                              <p className="text-orange-800 text-xs font-semibold">⚠️ Sin sesión {p.sinSesionDias}d</p>
                            </div>
                          )}
                          {p.tieneDeuda && (
                          ...
                          {!p.tieneDeuda && (p.sinSesionDias === null || p.sinSesionDias <= 30) && (
                            <p className="text-[#8a9b8e] text-xs">Sin alertas activas</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <Link href={`/admin/pacientes/${p.id}`}
                            className="w-full text-left px-3 py-2 rounded-xl bg-[#1a2e1e] text-white text-xs font-semibold flex items-center gap-2 hover:bg-[#2d4a32] transition-colors">
                            📋 Ver ficha completa
                          </Link>
                          <button onClick={() => resetPassword(p)}
                            className="w-full text-left px-3 py-2 rounded-xl bg-white border border-[#e8dfd0] hover:border-[#4a7c59]/40 text-xs text-[#1a2e1e] flex items-center gap-2 transition-colors">
                            🔑 Restablecer contraseña
                          </button>
                          <button onClick={() => window.location.href=`mailto:${p.email}`}
                            className="w-full text-left px-3 py-2 rounded-xl bg-white border border-[#e8dfd0] hover:border-[#4a7c59]/40 text-xs text-[#1a2e1e] flex items-center gap-2 transition-colors">
                            ✉️ Enviar email
                          </button>
                          {p.phone && (
                            <button onClick={() => window.open(`https://wa.me/${p.phone?.replace(/\D/g,'')}`, '_blank')}
                              className="w-full text-left px-3 py-2 rounded-xl bg-white border border-[#e8dfd0] hover:border-[#4a7c59]/40 text-xs text-[#1a2e1e] flex items-center gap-2 transition-colors">
                              💬 WhatsApp
                            </button>
                          )}
                          <button onClick={() => toggleActivo(p.id, p.isActive)}
                            className={`w-full text-left px-3 py-2 rounded-xl border text-xs flex items-center gap-2 transition-colors ${p.isActive ? 'bg-red-50 border-red-200 text-red-600 hover:bg-red-100' : 'bg-green-50 border-green-200 text-green-700 hover:bg-green-100'}`}>
